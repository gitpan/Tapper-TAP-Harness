# This Makefile is for the Tapper::TAP::Harness extension to perl.
#
...
#     ABSTRACT => q[Tapper - Tapper specific TAP handling]
#     AUTHOR => q[-2011 AMD OSRC Tapper Team, all rights reserved., AMD OSRC Tapper Team <tapper@amd64.org>]
#     BUILD_REQUIRES => { Test::Perl::Critic=>q[0], Test::More=>q[0], TAP::DOM=>q[0], Tapper::Test=>q[0], ExtUtils::MakeMaker=>q[6.42], File::Slurp=>q[0] }
#     DISTNAME => q[Tapper-TAP-Harness]
#     LICENSE => q[freebsd]
#     MIN_PERL_VERSION => q[5.010]
#     NAME => q[Tapper::TAP::Harness]
#     NO_META => q[1]
#     PREREQ_PM => { Directory::Scratch=>q[0], IO::Scalar=>q[0], Tapper::Test=>q[0], Test::Perl::Critic=>q[0], Archive::Tar=>q[0], File::Slurp=>q[0], Moose=>q[0], Test::Harness=>q[3.22], IO::String=>q[0], YAML::Tiny=>q[0], TAP::DOM=>q[0], ExtUtils::MakeMaker=>q[6.42], TAP::Parser=>q[0], TAP::Formatter::HTML=>q[0.04], TAP::Parser::Aggregator=>q[0], Test::More=>q[0] }
#     VERSION => q[3.000002]
#     VERSION_FROM => q[lib/Tapper/TAP/Harness.pm]
...
MAKE = make
FULLEXT = Tapper/TAP/Harness
BASEEXT = Harness
PARENT_NAME = Tapper::TAP
DLBASE = $(BASEEXT)
VERSION_FROM = lib/Tapper/TAP/Harness.pm
OBJECT = 
LDFROM = $(OBJECT)
LINKTYPE = dynamic
BOOTDEP = 
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Tapper/TAP/Harness.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Tapper/TAP
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Tapper/TAP
...
TO_INST_PM = lib/Tapper/TAP/Harness.pm

PM_TO_BLIB = lib/Tapper/TAP/Harness.pm \
...
DISTVNAME = Tapper-TAP-Harness-3.000002
...
manifypods : pure_all  \
	...
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Tapper/TAP/Harness.pm $(INST_MAN3DIR)/Tapper::TAP::Harness.$(MAN3EXT) 
...
ppd :
	...
	$(NOECHO) $(ECHO) '    <ABSTRACT>Tapper - Tapper specific TAP handling</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>-2011 AMD OSRC Tapper Team, all rights reserved., AMD OSRC Tapper Team &lt;tapper@amd64.org&gt;</AUTHOR>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <PERLCORE VERSION="5,010,0,0" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Archive::Tar" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Directory::Scratch" />' >> $(DISTNAME).ppd
	...
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="TAP::Formatter::HTML" VERSION="0.04" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="TAP::Parser" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="TAP::Parser::Aggregator" />' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '        <REQUIRE NAME="Test::Harness" VERSION="3.22" />' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Tapper/TAP/Harness.pm blib/lib/Tapper/TAP/Harness.pm 
